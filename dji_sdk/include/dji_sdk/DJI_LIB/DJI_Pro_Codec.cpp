...
void sdk_call_data_app(SDKFilter* p_filter)
{
  ...
  if (serial_hook)
  {
    sdk_stream_codec(p_filter, p_head, aes256_decrypt_ecb);

    serial_hook((SDKHeader*)p_filter->comm_recv_buf);
  }
...
void sdk_stream_update_reuse_part(SDKFilter* p_filter)
{
  ...
  p_filter->recv_index = sizeof(SDKHeader);
...
void sdk_stream_verify_head(SDKFilter* p_filter)
{
  SDKHeader* p_head = (SDKHeader*)(p_filter->comm_recv_buf);
  ...
  if ((p_head->sof == _SDK_SOF) &&
      ...
      (p_head->reversed0 == 0) &&
      (p_head->reversed1 == 0) &&
      (_SDK_CALC_CRC_HEAD(p_head, sizeof(SDKHeader)) == 0)
      )
  {
...
void sdk_check_stream_state(SDKFilter* p_filter)
{
  SDKHeader* p_head = (SDKHeader*)(p_filter->comm_recv_buf);

  if (p_filter->recv_index < sizeof(SDKHeader))
  {
...
unsigned short sdk_encrypt_interface(
